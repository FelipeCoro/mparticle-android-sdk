...
open class KitConfiguration {
    ...
    val settings = HashMap<String, String>(0)
    ...
    var highBracket = 101
        private set
    var customMappingList: LinkedList<CustomMapping>? = null
    ...
    open fun parseConfiguration(json: JSONObject): KitConfiguration {
        ...
        if (json.has(KEY_CONSENT_FORWARDING_RULES)) {
            ...
            for (i in 0 until rules.length()) {
                mConsentForwardingRules[rules.getJSONObject(i).getInt(
        ...
        if (isAttributeValueFilteringActive) {
            ...
            if (attributes != null) {
                ...
                while (attIterator.hasNext()) {
                    ...
                    val keyHash = KitUtils.hashForFiltering(key)
                    if (keyHash == avfHashedAttribute) {
                        val valueHash = KitUtils.hashForFiltering(value)
    ...
    fun isConsentStateFilterMatch(consentState: ConsentState?): Boolean {
        ...
        for ((key, value) in gdprConsentState) {
            val consentPurposeHash = KitUtils.hashForFiltering("1$key")
        ...
        if (ccpaConsent != null) {
            val consentPurposeHash =
                KitUtils.hashForFiltering("2" + Constants.MessageKey.CCPA_CONSENT_KEY)
            val consented = mConsentForwardingRules[consentPurposeHash]
    ...
    fun filterCommerceEvent(event: CommerceEvent): CommerceEvent? {
        ...
        if (eventTypeFilters != null &&
            !eventTypeFilters!![KitUtils.hashForFiltering(
            ...
            when (entity) {
                    ...
                    for (product in filteredEvent.products!!) {
                        ...
                        if (product.customAttributes != null && product.customAttributes!!.size > 0) {
                            ...
                            for ((key, value) in product.customAttributes!!) {
                                if (filters[KitUtils.hashForFiltering(key), true]) {
                                    filteredCustomAttributes[key] = value
                                }
                            }
                            productBuilder.customAttributes(filteredCustomAttributes)
                        }
                        if (!MPUtility.isEmpty(product.couponCode) && filters[KitUtils.hashForFiltering(
                        ...
                        if (product.position != null && filters[KitUtils.hashForFiltering(
                                CommerceEventUtils.Constants.ATT_PRODUCT_POSITION
                            ), true]
                        ) {
                        ...
                        if (!MPUtility.isEmpty(product.variant) && filters[KitUtils.hashForFiltering(
                                CommerceEventUtils.Constants.ATT_PRODUCT_VARIANT
                            ), true]
                        ) {
                        ...
                        if (!MPUtility.isEmpty(product.category) && filters[KitUtils.hashForFiltering(
                                CommerceEventUtils.Constants.ATT_PRODUCT_CATEGORY
                            ), true]
                        ) {
                        ...
                        if (!MPUtility.isEmpty(product.brand) && filters[KitUtils.hashForFiltering(
                                CommerceEventUtils.Constants.ATT_PRODUCT_BRAND
                            ), true]
                        ) {
                ...
                ENTITY_PROMOTION -> if (filteredEvent.promotions != null && filteredEvent.promotions!!.size > 0) {
                    ...
                    for (promotion in filteredEvent.promotions!!) {
                        ...
                        if (!MPUtility.isEmpty(promotion.id) && filters[KitUtils.hashForFiltering(
                                CommerceEventUtils.Constants.ATT_PROMOTION_ID
                            ), true]
                        ) {
                            filteredPromotion.id = promotion.id
                        }
                        if (!MPUtility.isEmpty(promotion.creative) && filters[KitUtils.hashForFiltering(
                                CommerceEventUtils.Constants.ATT_PROMOTION_CREATIVE
                            ), true]
                        ) {
                            filteredPromotion.creative = promotion.creative
                        }
                        if (!MPUtility.isEmpty(promotion.name) && filters[KitUtils.hashForFiltering(
                                CommerceEventUtils.Constants.ATT_PROMOTION_NAME
                            ), true]
                        ) {
                            filteredPromotion.name = promotion.name
                        }
                        if (!MPUtility.isEmpty(promotion.position) && filters[KitUtils.hashForFiltering(
    ...
    fun filterEventAttributes(
        ...
        return if (eventAttributes != null && eventAttributes.size > 0 && filter != null && filter.size() > 0) {
            ...
            while (attIterator.hasNext()) {
                val (key, value) = attIterator.next()
                val hash = KitUtils.hashForFiltering(eventTypeStr + eventName + key)
                if (filter[hash, true]) {
        ...
        if (removeProducts) {
            ...
            if (impressionList != null) {
                ...
                for (impression in impressionList) {
                    builder.addImpression(Impression(impression.listName, null))
                }
            }
        }
        if (removePromotions) {
    ...
    private fun filterCommerceEventAttributes(filteredEvent: CommerceEvent): CommerceEvent {
        ...
        if (customAttributes != null) {
            ...
            for ((key, value) in customAttributes) {
                if (commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + key), true]) {
                    filteredCustomAttributes[key] = value
                }
            }
        ...
        if (filteredEvent.checkoutStep != null &&
            !commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + CommerceEventUtils.Constants.ATT_ACTION_CHECKOUT_STEP), true]
        ) {
            builder.checkoutStep(null)
        }
        if (filteredEvent.checkoutOptions != null &&
            !commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + CommerceEventUtils.Constants.ATT_ACTION_CHECKOUT_OPTIONS), true]
        ) {
        ...
        if (attributes != null) {
            if (attributes.couponCode != null &&
                !commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + CommerceEventUtils.Constants.ATT_TRANSACTION_COUPON_CODE), true]
            ) {
                attributes.couponCode = null
            }
            if (attributes.shipping != null &&
                !commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + CommerceEventUtils.Constants.ATT_SHIPPING), true]
            ) {
                attributes.shipping = null
            }
            if (attributes.tax != null &&
                !commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + CommerceEventUtils.Constants.ATT_TAX), true]
            ) {
                attributes.tax = null
            }
            if (attributes.revenue != null &&
                !commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + CommerceEventUtils.Constants.ATT_TOTAL), true]
            ) {
                attributes.revenue = 0.0
            }
            if (attributes.id != null &&
                !commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + CommerceEventUtils.Constants.ATT_TRANSACTION_ID), true]
            ) {
                attributes.setId(null)
            }
            if (attributes.affiliation != null &&
                !commerceAttributeFilters!![KitUtils.hashForFiltering(eventType + CommerceEventUtils.Constants.ATT_AFFILIATION), true]
            ) {
    ...
    fun shouldLogScreen(screenName: String): Boolean {
        val nameHash = KitUtils.hashForFiltering("0$screenName")
    ...
    fun shouldLogEvent(event: MPEvent): Boolean {
        ...
        val typeHash = KitUtils.hashForFiltering(event.eventType.ordinal.toString() + "")
    ...
    fun shouldSetIdentity(identityType: IdentityType): Boolean {
    ...
    companion object {
        ...
        fun filterAttributes(
            ...
            return if (attributes != null && attributeFilters != null && attributeFilters.size() > 0 && attributes.isNotEmpty()) {
                val newAttributes = HashMap<Any?, Any?>(attributes.size)
                for ((key, value) in attributes) {
                    if (shouldForwardAttribute(attributeFilters, key)) {
                        newAttributes[key] = value
                    }
                }
                newAttributes as Map<String, Any?>
            } else {
                attributes
            }
        }
        ...
        fun shouldForwardAttribute(attributeFilters: SparseBooleanArray, key: String?): Boolean {
            val hash = KitUtils.hashForFiltering(key)
