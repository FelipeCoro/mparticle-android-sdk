...
import com.mparticle.testutils.*
...
class MParticleJSInterfaceITest : BaseCleanStartedEachTest() {
    @Rule
    ...
    fun before() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            Handler(Looper.getMainLooper()).post { WebView.setWebContentsDebuggingEnabled(true) }
        }
    }
    ...
    fun testSetUserAttribute() {
        val key = mRandomUtils.getAlphaNumericString(25)
        val value = mRandomUtils.getAlphaNumericString(25)
    ...
    fun testRemoveUserAttribute() {
        val key = mRandomUtils.getAlphaNumericString(20)
        val testJavascript =
    ...
    @Throws(Exception::class)
    fun testSetUserTag() {
        val tag = mRandomUtils.getAlphaNumericString(25)
        val testJavascript =
    ...
    @Throws(Exception::class)
    fun testLogEvent() {
        val customAttributes = MPUtility.mapToJson(mRandomUtils.getRandomAttributes(10))
            ...
            override fun logEvent(json: String) {
                ...
                try {
                    ...
                    if (jsonObject.getInt(JS_KEY_EVENT_DATATYPE) == JS_MSG_TYPE_PE) {
                        ...
                        TestingUtils.assertJsonEqual(customAttributes, receivedCustomAttributes)
                        TestingUtils.assertJsonEqual(customFlagsJSON, receivedCustomFlags)
                        called.value = true
                        latch.countDown()
                    }
...
mParticle.eCommerce.logPurchase(transactionAttributes, product, true, %s, %s);""",
        ...
        val called = MutableBoolean(false)
        val error = AndroidUtils.Mutable<Any?>(null)
        val latch: CountDownLatch = MPLatch(2)
        runJavascriptTest(testJavascript, object : MParticleJSInterface() {
            override fun logEvent(event: BaseEvent) {
                val customFlags = event.customFlags
            ...
            override fun logEvent(json: String) {
                ...
                try {
                    ...
                    TestingUtils.assertJsonEqual(
                        customAttributes,
                        MPUtility.mapToJson(commerceEvent.customAttributeStrings)
                    )
                    called.value = true
                } catch (e: Exception) {
                    error.value = e
    ...
    @Throws(Exception::class)
    fun testLogout() {
        val userIdentityMap = mRandomUtils.randomUserIdentities
    ...
    private fun runJavascriptTest(testJavascript: String, jsInterface: MParticleJSInterface) {
        Handler(Looper.getMainLooper()).post {
            ...
            rule.scenario.onActivity { activity: WebViewActivity ->
    ...
    private fun userIdentityMapToJson(userIdentities: Map<IdentityType, String>): JSONObject {
    ...
    @Throws(JSONException::class)
    private fun userIdentityMapToJsonJsSdkStyle(userIdentities: Map<IdentityType, String>): JSONObject {
    ...
    internal object OptionsAllowResponse {
        ...
        fun build(): WebResourceResponse {
            ...
            val headers= object : HashMap<String, String>() {
                init {
    ...
    companion object {
        ...
        private val bridgeToken = RandomUtils().getAlphaString(5)
