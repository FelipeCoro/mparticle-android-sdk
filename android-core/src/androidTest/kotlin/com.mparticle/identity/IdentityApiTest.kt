...
import com.mparticle.networking.MockServer.RequestReceivedCallback
...
class IdentityApiTest : BaseCleanStartedEachTest() {
    ...
    fun testIdentityStateListenerThread() {
        ...
        Handler(backgroundThread.looper).post {
            MParticle.getInstance()!!.Identity().addIdentityStateListener { user, previousUser ->
                ...
                Assert.assertEquals(
                    user.id, MParticle.getInstance()!!.Identity().currentUser!!
                        .id
                )
                called.value = true
                latch.countDown()
    ...
    fun testCallbacksVsCurrentUser() {
        ...
        MParticle.getInstance()!!.Identity().login()
            .addSuccessListener { result ->
                ...
                Assert.assertEquals(
                    mStartingMpid.toLong(), result.previousUser!!
                        .id
                )
                called2.value = true
                latch.countDown()
    ...
    fun testGetUser() {
        ...
        AccessUtils.setUserIdentities(
            mpid1UserIdentites, identity.currentUser!!
                .id
        )
        ...
        AccessUtils.setUserIdentities(
            mpid2UserIdentites, identity.currentUser!!
                .id
        )
        ...
        AccessUtils.setUserIdentities(
            mpid3UserIdentities, identity.currentUser!!
                .id
        )
    ...
    fun testModifyConcurrentCalls() {
        Assert.assertEquals(
            mStartingMpid.toFloat(), MParticle.getInstance()!!
                .Identity().currentUser!!.id.toFloat(), 0f
        )
    ...
    fun testModifyDoesntMissIdentitiesSetNull() {
        ...
        mServer.waitForVerify(Matcher(mServer.Endpoints().getModifyUrl(mStartingMpid))) { request ->
            ...
            Collections.sort(identityChanges) { jsonObject, t1 ->
